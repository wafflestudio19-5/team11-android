    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        commentAdapter.setItemClickListener(object : CommentAdapter.OnCommentClickListener {
            ...
            override fun onCommentClick(parent: Int) {
                val mBuilder = AlertDialog.Builder(this@ArticleActivity)
                    ...
                    .setPositiveButton("확인") { dialogInterface, i ->
                        ...
                        binding.commentView[commentAdapter.getItemPosition(parent)].setBackgroundColor(
            ...
            override fun onCommentLikeClick(id: Int) {
                if (id == -1) {
                    Toast.makeText(this@ArticleActivity, "내가 쓴 댓글은 공감할 수 없습니다", Toast.LENGTH_SHORT)
                        .show()
                } else {
                    val mBuilder = AlertDialog.Builder(this@ArticleActivity)
        ...
        viewModel.deleteArticleResult.observe(this, {
            if (it == "success") {
                // 잘 삭제 됐으면 액티비티 종료, 그리고 BoardActivity 가 새로고침 하도록 result 설정
                setResult(RESULT_OK)
                finish()
            } else {
                // 삭제 과정에 오류가 있으면 출력
